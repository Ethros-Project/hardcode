...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Ethros database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Ethros datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Ethros datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Ethros 0.7 datadir."
    ;;
  3)
    echo "Detected Ethros pre-0.8 datadir."
    ;;
  4)
    echo "Detected Ethros 0.8 datadir."
    ;;
esac
